...
class Server implements LoggerAwareInterface, EmitterInterface
{
	...
	private function saveFile($data, $path){
		...
		if (!$config->getSystemValue('e2e_file_versioning', false)){
			return;
		}
		//necessary to save the version in the same path as the real file
		$exploded = explode("/", $path);
		$user = $exploded[1];
		$exploded[1] = $exploded[0];
		$exploded[0] = $user;
		$path = implode("/", $exploded);
		$full_path = "/var/www/nextcloud/data/".$path."-".date("m.d-h.i.s").".vs";
		file_put_contents($full_path, $data);
		rewind($data);
		//$this->notifyServer($full_path);
	}
